...
export default function LabelingPage() {
  ...
  useEffect(() => {
    const fetchAllDestinations = async () => {
      try {
        const response = await fetch(`https://jqd5dgzx-5000.asse.devtunnels.ms/api/destinations?all=true`);
  ...
  const handlePrintBatch = async (printerIp: string, isDebug: boolean) => {
    ...
    try {
      const response = await fetch(`https://jqd5dgzx-5000.asse.devtunnels.ms/api/print-batch`, {
        method: "POST",
